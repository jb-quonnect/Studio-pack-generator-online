...
- [app.py](app.py) — Monolithe Streamlit : deux parcours pilotés par `st.session_state.view` (`home`/`create`/`lunii`), barre d'onglets maison. **« Créer un pack »** : import (`render_input_tabs`) → aperçu/éditeur → téléchargement → conversion Lunii. **« Ma Lunii »** (`render_lunii_view`) : gestionnaire d'appareil autonome + import d'un ZIP Studio/Lunii (conversion auto via `_prepare_lunii_pack`) prêt à installer. Point d'entrée `main()`. Téléchargements servis par `serve_download_link()`/`_write_download_file()` (fichiers statiques, cf. Pièges connus).
...
- [static/lunii_manager.js](static/lunii_manager.js) — Gestionnaire d'appareil Lunii côté navigateur (File System Access API), injecté via `components.html` depuis app.py : détection V2/V3, liste/réordonnancement/suppression/installation des packs. Installe un ZIP soit choisi localement (`<input type=file>`), soit **préparé côté serveur et récupéré par `fetch` d'URL** (`installPrepared()`, via `window.LUNII_AUTOINSTALL` — évite d'embarquer un gros ZIP dans la page). Basé sur l'architecture d'olup/lunii-admin-web. Le chiffrement XXTEA y est réimplémenté en JS et doit rester cohérent avec la version Python de lunii_converter.py.
...
- **Règle d'or navigation** : tout nœud avec `autoplay = true` DOIT avoir une transition OK valide — le firmware déclenche automatiquement la transition OK en fin d'audio ; une transition OK nulle = **crash (icône erreur) à la fin de l'épisode**. Bug corrigé en juillet 2026 dans l'expansion `storyAudio` de [lunii_converter.py](modules/lunii_converter.py) (les nœuds de lecture pointaient OK vers rien) + filet de sécurité dans `generate_ni()`. Détails et preuve : docs/LUNII-FORMAT.md §7.
- **État vérifié (juillet 2026)** : structure/index/BMP/MP3/bt corrects (contrôlés octet par octet sur une Lunii V2 et par conversion réelle). Divergences résiduelles avec la référence Java (non bloquantes) : pas de dédoublonnage SHA1 des assets, 1 s de silence ajoutée à chaque audio, `BLANK_MP3` minuscule non validé. (Le bitrate est passé à 64k, aligné sur la référence.)
...
- **Limite 200 Mo du service statique Streamlit** : le service de fichiers statiques refuse tout fichier **> 200 Mo** avec un HTTP 404 (`MAX_APP_STATIC_FILE_SIZE`, constante non configurable). Les gros packs Lunii dépassent ce seuil. Deux parades en place : (1) `_raise_streamlit_static_limit()` en tête d'[app.py](app.py) relève la limite à 4 Go par monkey-patch (best-effort, s'exécute à chaque run de script donc avant tout clic) ; (2) l'audio Lunii est encodé en **64 kbps** (et non 128k) — format recommandé par la référence, qui divise la taille par ~2. Si une future montée de version Streamlit casse le patch, le 64k garde les packs courants sous la limite.
